...
func DetectFrontMatter(mark rune) (f *FrontmatterType) {
	...
	case '+':
		return &FrontmatterType{[]byte(HEAD_DELIM), []byte(HEAD_DELIM), HandleHeadMetaData, false}	
	case '{':
...
func HandleHeadMetaData(datum []byte) (interface{}, error) {
	m := map[string]interface{}{}
	datum = removeHeadIdentifier(datum)

	// TODO: parse head meta data
	lines := bytes.Split(datum, []byte("\n"))
	for _, line := range lines {
		line = bytes.TrimPrefix(line, []byte("\r"))
		line = bytes.TrimSuffix(line, []byte("\r"))
		idx := bytes.IndexRune(line, rune('='))
		if idx == -1{
			continue
		}
		if len(line) > (idx + 1) {
			m[string(line[0:idx])] = string(line[idx+1:])
		}	
	}
...
func removeHeadIdentifier(datum []byte) []byte {
	return bytes.Replace(datum, []byte(HEAD_DELIM), []byte(""), -1)
}
